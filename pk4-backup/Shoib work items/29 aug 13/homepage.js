...
function addHomepageScripts() {
  ...
  //http://datatables.net/media/javascript/complete.min.js
    ...
    // $("body").on('mouseover', '.firstLevelMenu_li', function() {
    //   var id = $(this).attr('id').split('_');
    //       id = id[1];
    //   var subId = 'secondLevelMenuUL_'+id; 
    //   $('.secondLevelMenu_li').css('display', 'none'); //first display none all
    //   //then show current hover tab
    //   $('#'+subId+','+'#'+subId+' li').css('display', 'block');
    // });

     // click event
     var tempTabId;
  	 $("body").on('click', 'a.mnuSpan', function() {
  			var hash = this.href;
  			hash = hash.replace(/^.*#/, '');
        var id = $(this).attr('id').split('_');
        tempTabId = id[1];
    ...
    //  //attaching tab click event
     ...
     $('body').on('click', '.secondLevelMenu_li', function() {
        var cid = $(this).parent().attr('id').split('_');
          ...
          console.log('#mnuList_'+cid);
     });

    	

     // $("body").on('click', 'a.lineMnuLiAnc', function() {
     ...
     // });  
  	 ...
  	 // $(document).on('click', 'a.lineMnuLiAnc', function() {
    //   alert('tab');
    ...
    .script("/atCRM/javascript/jquery/pivot/gchart_renderers.js").wait()
    .wait(function() {
