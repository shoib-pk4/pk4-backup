...
function populatePicklist(entity,name,elemId,strVal,txtbxval,elem,pckLstURL,pckLstTitle,udmName,paramName,readNodeId)
{ 		
	document.getElementById("commonPickListDiv").innerHTML = "";
	//Open the common popup div if its not open
	if(url4PckLst=='')
	{
		$('#commonPickListDiv').dialog({
			autoOpen:true,
			modal: false,
			title:'Picklist - ',
			position: 'center',
	...
	myElement=document.getElementById(elemId);myElementId=elemId;myElementVal=myElement.value;gblPckEntity=entity;gblPckName=name;
	...
	{		
		...
		success: function (data)
		{
			...
			//alert(width4Popup);
			$("#commonPickListDiv").dialog( "option", "width", width4Popup);
			...
			document.getElementById('ui-dialog-title-commonPickListDiv').innerHTML=pckListTitle;
...
function writeTable(pckLstURL)
{
	...
	{
		var commonPickListDiv=document.getElementById('commonPickListDiv');
		...
		AddClickEventListener(pckHref,function (){populatePicklist(gblPckEntity,gblPckName,myElementId,'*');});
...
function reWritePckList(elem,event,pckLstURL)
{
	...
	if (e==13)
	{
		...
		if(strData!='')populatePicklist(gblPckEntity,gblPckName,myElementId,strData);
	}
}
...
function selectPcklstItm(Id,val,PckName)
{
	...
	$("#commonPickListDiv").dialog("close");
...
{ 
	...
	if(pgNo<=max&&pgNo>0)
	{
		...
		populatePicklist(gblPckEntity,gblPckName,myElementId,searchParam,'','','','','',paramName,readNodeId);
	}
}