...
function retrieveListData(action,paramVal,DispTxt) {
	var url2call;
	...
	if (container_json.addl_options == "anyList") {
		...
		if (typeof data_view_id_string == "undefined") {
			var reptId=document.getElementById(subMnuItmId+'-Title').value.split('--')[0];
			var adQuery=document.getElementById(subMnuItmId+'-Title').value.split('--')[2];
		} else {
			var reptId = data_view_id_string.split('--')[0];
...
/*  Hack: Fix to deal with report ID not being passed to data views  - Kishore, 5 Jun 2013 */
	...
	if (document.getElementById(subMnuItmId+'-reportUrl').value == "" || document.getElementById(subMnuItmId+'-reportUrl').value != url) {
...
/*Added by DPVenkatesh to add View as Menu Item for default user and selected user*/
/*---- Date : 19-june-2013 ----*/
...
/*end of adding View as Menu Item*/
...
function listPageMenuItems(data) {
	...
	topMenuDiv.innerHTML+='<li style="width:140px;"><input type="text" class="searchBox" id="'+subMnuItmId+'searchTxt"  name="'+subMnuItmId+'searchTxt" style="margin: 0px;width:120px;" value="'+srchBoxDispTxt+'" onfocus="if(this.value==\''+srchBoxDispTxt+'\')this.value=\'\'" onblur="if(this.value==\'\'){this.value=\''+srchBoxDispTxt+'\'}" onkeypress="{var charCode = event.keyCode ? event.keyCode :event.which ? event.which : event.charCode; if (charCode==13&&this.value!=\'\')retrieveListData(\'search\',this.value);}"><img  id="'+subMnuItmId+'searchImg" src="/atCRM/images/JSON/close_gray.png" style="z-index: 1; position: absolute; margin-left: -12px;padding: 7px 0 0 0;cursor:pointer;visibility:hidden" onclick="javascript:retrieveListData(\'search\',\'\',\''+srchBoxDispTxt+'\');" title="Clear Search"></li>';
			...
			for(var cols=-1; cols<ColumnHeaderLength; cols++) {
				...
				/*	var listDataTd = CreateTD(listDataTr);
					//if (container_json.addl_options == 'anyList') {
						...
						var lineMenuUL = CreateUL(lineLevelMnu,'lineMnuUL');
							for (ix = 0; ix < RowData[rows].menus.length; ix++) {
								...
								lineMenuUL.style.border="red";
								if(mnuUrl.indexOf(":menuId:")>0){ mnuUrl.replace(":menuId:",entityDiv);}
								if(mnuUrl.indexOf(":subMnuId:")>0) mnuUrl.replace(":subMnuId:",shownSubMenu);
								var lineMenuLi = CreateLI (lineMenuUL,'lineMnuLi');
								var lineMnuAnc = CreateA (lineMenuLi, "lineMnuLiAnc", "", mnuUrl, null,mnuName,mnuName);
								if(mnuName == 'view') viewExist ="1";							}
							if (ix > 0) {
					...
					switch(dataType) {
						...
						case "timeOnly": case "dateAndTime": case "DateTime":
										txtAlign='center';
										var d1 = parseString_Date(listDataTxt,'yyyy-MM-dd');										
										d1.setMinutes ( d1.getMinutes() - srvrtimeDiffInMin + timeDiffInMin);
				...
				if(cols!=-1) {
					...
					//if (container_json.addl_options == 'anyList') {
						if (typeof RowData[rows].menus != "undefined") {
							for (ix = 0; ix < RowData[rows].menus.length; ix++) {
								...
								if(mnuUrl.indexOf("::tblName::")>0) mnuUrl=mnuUrl.replace("::tblName::",data.TableName);
								if(mnuUrl.indexOf("::enttName::")>0) mnuUrl=mnuUrl.replace("::enttName::",currEntt);
								var lineMnuAnc = CreateA (lineMenuLi, "lineMnuLiAnc", "", mnuUrl, null, mnuName,mnuName);
								if(mnuName == 'view') viewExist ="1";
							}
							if (ix > 0) {
					...
					/*if(data.ViewUrl){var viewParam=(data.ViewUrl).indexOf('?');if(viewParam>0)var viewUrl=data.ViewUrl+"&id="+RowData[rows].pkId+"&tableName="+data.TableName+"&entityName="+currEntt;else var viewUrl=data.ViewUrl+"?id="+RowData[rows].pkId+"&tableName="+data.TableName+"&entityName="+currEntt;}*/
					...
					if(showView==true && viewUrl && viewExist !="1") {
						var lineMenuLi=CreateLI(lineMenuUL,'lineMnuLi');	
				...
				for(var colmnns = -1; colmnns < ColumnHeaderLength; colmnns++) {
					...
					if(colmnns != -1) {
						...
						for (ix = 0; ix < RowData[row].menus.length; ix++) {
							...
							if(mnuUrl.indexOf("::enttName::")>0) mnuUrl=mnuUrl.replace("::enttName::",currEntt);
							var lineMnuAnc = CreateA (lineMenuLi, "lineMnuLiAnc", "", mnuUrl, null, mnuName,mnuName);
							if(mnuName == 'view') chkViewExist ="1";
						//	var lineMnuAnc = CreateA (lineMenuLi, "lineMnuLiAnc", "", RowData[row].menus[ix].url, null, RowData[row].menus[ix].name,RowData[row].menus[ix].name);
						}
						if (ix > 0) {
					...
					// }
						...
						if (data.ViewUrl){
							...
							if(viewParam>0)
								//var viewUrl=data.ViewUrl+"&id="+RowData[row].pkId+"&tableName="+data.TableName+"&entityName="+currEntt;
