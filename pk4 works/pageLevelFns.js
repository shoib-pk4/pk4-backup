...
function addOrderPageSetUp(myForm,contId,acctId)
{
	...
	if(contId && document.getElementById('0-1-4001hdn'))
	{
		document.getElementById('0-1-4001hdn').value=contId;
		blockMappedObjectFields('0-1-4001','0-1-4001hdn');
	}
	else if(acctId && document.getElementById('0-1-1901hdn'))
	{
...
{ 
	for(var x=0; x<rows; x++)
	{
		...
		if(document.getElementById('0-1501:'+x+'-401-2201'))
...
{ 
	...
	if(listPriceObj.value!="")
	{
		...
		if(totalPriceObj)totalPriceObj.value=finalAmount.toFixed(2);
...
var reqPageURI="";
var blockDtl="";
var deleteRowArr = new Array();
function addInvoicePageSetUp(myForm,autoOrgflag)
{
//	callAllTaxItems();
	firstLoad="1";    //set firtload to 1 on open of add/edit page.
   ...
   // callAllTaxItems();
   /*If 'tax_at_detail_level_not_header' flag is set make tax type value to null else call the function callAllTaxItems()*/
	showHeaderTax = "1";
	for(var t=0;t<orgFlagData.length;t++)
	 {
		var flagName = orgFlagData[t]["name"];
		var flagVal = orgFlagData[t]["flag_value"];
		if(flagName == "tax_at_detail_level_not_header" && flagVal=="1")
		 {
		    showHeaderTax = "0";
		 }
	}
	if(showHeaderTax == "1") callAllTaxItems();
	else{
     if(document.getElementById('0-1-39')) document.getElementById('0-1-39').value="";
	 if(document.getElementById('0-1-46')) document.getElementById('0-1-46').value="";
	 if(document.getElementById('0-1-53')) document.getElementById('0-1-53').value="";
	 if(document.getElementById('0-1-60')) document.getElementById('0-1-60').value="";
	 if(document.getElementById('0-1-67')) document.getElementById('0-1-67').value="";
	}
}
/**This function will block the invoice detial columns which are mentioned in the array**/
...
var priceInclTaxFlg =""; 
function calculateTotForInv(myForm,Obj,isRowDelete)
{ 
	var flagData = flagsJSON;  //'flagsJSON' is a global variable
	var orgFlagData = flagData[0]["org_flags"];
	for(var t=0;t<orgFlagData.length;t++)
	...
	if(document.getElementById('0-1-851'))if((document.getElementById('0-1-851').value=="")||(isNaN(document.getElementById('0-1-851').value)))
    ...
    discSqlRepId ="";
	/*If quantity element called the above function*/
	if(elemId == '0-1001:'+srcZcRank+'-1101-1103' && document.getElementById(elemId).value)
    {
		...
		/*If edit invoice page, validate the qty and store the dtlId,qty in the array */
		if(invQtyArray[srcZcRank] && priKeyVal && priKeyVal!="" && isRowDelete !="1")
		{
            var oldQtyArr = invQtyArray;
			if(oldQtyArr[srcZcRank]<prodQty){
				alert("Quantity can not be greater than the previous value");
				noOfUnitsObj.value = oldQtyArr[srcZcRank];
				noOfUnitsObj.focus();
				return;
			}
			else if(prodQty == 0){
			...
			else if(oldQtyArr[srcZcRank]>prodQty){
				var invDtlId = document.getElementById("0-1001:"+srcZcRank+"-1101-1102").value;
				invDtlId=invDtlId.replace(/,/g,"");
                if(qtyUdtDtlCSV=="") qtyUdtDtlCSV = invDtlId+"~"+prodQty;
                else qtyUdtDtlCSV = qtyUdtDtlCSV+","+invDtlId+"~"+prodQty;
			}
		}
		/*Edit page quantity validation ends here*/
		var flagData = parent.flagsJSON;
		var orgFlagData = flagData[0]["org_flags"];
		/*check for 'line_level_discount_engine_sql_query_id' org_flag.
		If set get the report Id from the flag .*/
		for(var t=0;t<orgFlagData.length;t++)
		{
			var flagName = orgFlagData[t]["name"];
			var flagVal = orgFlagData[t]["flag_value"];
		...
		/*If 'line_level_discount_engine_sql_query_id' flag is set do the discount calculation*/
		if(discSqlRepId && discSqlRepId !="")
		{
            currentRecIdsJSON["rep_id"] = discSqlRepId;
			var curRecJSON = currentRecIdsJSON;
			var brchId ;
			var stateId;var prodId;
			...
			if(curRecJSON["prod_id"]) prodId=curRecJSON["prod_id"];
			else if(document.getElementById("0-1001:"+srcZcRank+"-1101-2201hdn")) prodId= document.getElementById("0-1001:"+srcZcRank+"-1101-2201hdn").value;
					 ...
					 for(var t=0;t<25;t++)
					   {
						  if(document.getElementById("0-1001:"+t+"-1101-1125") && document.getElementById("0-1001:"+t+"-1101-1125").value!="") {
                              ...
                              totAmount = parseFloat(totAmount)+parseFloat(extndAmt);
						  } 
					   }
					 if(acctType == "corporate")
					   {
						 if(document.getElementById("totalPrice"))
						   {
						       if(parseFloat(totAmount) >1000)
							   {
							      if(document.getElementById("0-1-851")&&(document.getElementById("0-1-851").value=="" ||document.getElementById("0-1-851").value==0)) document.getElementById("0-1-851").value = "15";
                                  calculateProdTotalPriceForInvoice(myForm, "0-1-851","","grndTot");
								  toGetTaxValues(srcZcRank);
								  showPrice(srcZcRank)
							   }
							   else if(parseFloat(totAmount) > 500) 
							   {
                                  if(document.getElementById("0-1-851")&&document.getElementById("0-1-851").value=="") document.getElementById("0-1-851").value = "10";
                                  calculateProdTotalPriceForInvoice(myForm, "0-1-851","","grndTot"); // pass 'grndTot' flag to make grandTotalPrice and totalPrice same
								  toGetTaxValues(srcZcRank);     //calculate the tax based on the values changed
								  showPrice(srcZcRank)          // recalculate the actual price (price exclusive of tax)
							   }
							   else{
								   if(document.getElementById("0-1-851") && (document.getElementById("0-1-851").value == ""||document.getElementById("0-1-851").value==0)) document.getElementById("0-1-851").value = "0";
                                   calculateProdTotalPriceForInvoice(myForm, "0-1-851","","grndTot");
								   toGetTaxValues(srcZcRank);
								   showPrice(srcZcRank)
							   }
					   ...
					   { 
							   ...
							   {
								 if(document.getElementById("0-1-851")&&(document.getElementById("0-1-851").value==""||document.getElementById("0-1-851").value==0)) document.getElementById("0-1-851").value = "10";
								 calculateProdTotalPriceForInvoice(myForm, "0-1-851","","grndTot");
								 toGetTaxValues(srcZcRank);
								 showPrice(srcZcRank)
							   }
							   else{
								   if(document.getElementById("0-1-851") && document.getElementById("0-1-851").value == "") document.getElementById("0-1-851").value = "0";
                                   calculateProdTotalPriceForInvoice(myForm, "0-1-851","","grndTot");
								   toGetTaxValues(srcZcRank);
								   showPrice(srcZcRank)
							   }
		 ...
		 if(actualPriceInclTaxObj && actualPriceInclTaxObj.value !="" && actualPriceObj && noOfUnitsObj && noOfUnitsObj.value !="")
			{
              ...
              if(unitQty !=0) actualPriceObj.value = (ReplaceAll(priceInclTax,",",""))-(txtAmt/unitQty);
              else if(unitQty ==0) actualPriceObj.value = (ReplaceAll(priceInclTax,",",""));
		...
		if(listPriceObj&&listPriceObj.value!="")
		{
			...
			else if(priceInclTaxFlg == "1" && actualPriceInclTaxObj)
			{
				...
				if(actualPriceInclTaxObj && actualPriceInclTaxObj.value !="" && actualPriceObj && noOfUnitsObj && noOfUnitsObj.value !="")
				{
				  ...
				  var unitQty = noOfUnitsObj.value;
				  var priceInclTax = actualPriceInclTaxObj.value;
				  if(unitQty !=0) actualPriceObj.value = ((ReplaceAll(priceInclTax,",",""))-parseFloat(txtAmt/unitQty)).toFixed(2);
				  else if(unitQty ==0) actualPriceObj.value = (ReplaceAll(priceInclTax,",",""));
				  actualPriceObj.disabled =true;
				  totalPriceObj.disabled =true;
				}
...
function calculateProdTotalPriceForInvoice(myForm, Obj,from_unblock,chngGrndTot)
{
	...
	if(from_unblock==1)
	{
		for (var i in prodTotalAmnt) 
		{
			...
			{	
				...
				document.getElementById('grandTotalPrice').value=parseFloat(totalAmntOfProds)+parseFloat(document.getElementById('0-1-45').value)
				parseFloat(document.getElementById('0-1-52').value)+parseFloat(document.getElementById('0-1-59').value)+parseFloat(document.getElementById('0-1-66').value)+parseFloat(document.getElementById('0-1-73').value);
			}
			else
			{
				document.getElementById('totalPrice').value = totalAmntOfProds.toFixed(2);
	...
	{	
		...
		if(document.getElementById('0-1-850') && document.getElementById('0-1-850').value!=""){
			var discVal = document.getElementById('0-1-850').value;
			discVal = discVal.replace(/Rs./gi,"");
			document.getElementById('0-1-850').value=discVal;
		}
		/*check if the discount amnt has the value on load of the page (to check the edit case). 
		If so dont calculate the amount. Instead assign the existing value.*/
		if(firstLoad=='1' && document.getElementById('0-1-850') && document.getElementById('0-1-850').value !="") document.getElementById('0-1-850').value=document.getElementById('0-1-850').value;
		else if(document.getElementById('0-1-850'))document.getElementById('0-1-850').value=((parseFloat(totalAmntOfProds)*(parseFloat(discPct)))/100);
		if(firstLoad=='1' && document.getElementById('0-1-850') && document.getElementById('0-1-850').value !="") totalAmntOfProds=((parseFloat(totalAmntOfProds))-(document.getElementById('0-1-850').value));
		else totalAmntOfProds=((parseFloat(totalAmntOfProds))-((parseFloat(totalAmntOfProds)*(parseFloat(discPct)))/100));
		document.getElementById('totalPrice').value=totalAmntOfProds.toFixed(2);
		firstLoad = "0";
	}
   ...
   /*If this function called from discount calculation section (if 'line_level_discount_engine_sql_query_id' flag is set) 
   and if the grandTotalPrice and total price is same*/
...
function calDisAmtTotalPrice4Inv(discAmt)
{
	...
	if(DiscountAmt!=discAmt)
	{
		...
		if(discAmt&&document.getElementById('totalPrice')&&document.getElementById('totalPrice').value!="")
		{
			...
			if(document.getElementById('0-1-45') && isNaN(document.getElementById('0-1-45').value))document.getElementById('0-1-45').value=0;
			if(document.getElementById('0-1-52') && isNaN(document.getElementById('0-1-52').value))document.getElementById('0-1-52').value=0;
			if(document.getElementById('0-1-59') && isNaN(document.getElementById('0-1-59').value))document.getElementById('0-1-59').value=0;
			if(document.getElementById('0-1-66') && isNaN(document.getElementById('0-1-66').value))document.getElementById('0-1-66').value=0;
			if(document.getElementById('0-1-73') && isNaN(document.getElementById('0-1-73').value))document.getElementById('0-1-73').value=0;
			var grandTotalAmt = totalAmt+parseFloat(document.getElementById('0-1-45').value)
			if(document.getElementById('0-1-52')) grandTotalAmt += parseFloat(document.getElementById('0-1-52').value);
			if(document.getElementById('0-1-59')) grandTotalAmt += parseFloat(document.getElementById('0-1-59').value);
			if(document.getElementById('0-1-66')) grandTotalAmt += parseFloat(document.getElementById('0-1-66').value);
			if(document.getElementById('0-1-73')) grandTotalAmt += parseFloat(document.getElementById('0-1-73').value);
	...
	if(DiscountPercent!=discPct)
	{
		...
		else
		{
			...
			if(discPct&&document.getElementById('totalPrice')&&document.getElementById('totalPrice').value!="")
			{
				...
				if(document.getElementById('0-1-52') && isNaN(document.getElementById('0-1-52').value))document.getElementById('0-1-52').value=0;
				if(document.getElementById('0-1-59') && isNaN(document.getElementById('0-1-59').value))document.getElementById('0-1-59').value=0;
                var tax1Val = 0;var tax2Val = 0;var tax3Val = 0;
                if(document.getElementById('0-1-45')) tax1Val=document.getElementById('0-1-45').value; else tax1Val=0;
                if(document.getElementById('0-1-52')) tax2Val=document.getElementById('0-1-52').value; else tax2Val=0;
                if(document.getElementById('0-1-59')) tax3Val=document.getElementById('0-1-59').value; else tax3Val=0;
				var grandTotalAmt = parseFloat(totalAmntOfProds)+parseFloat(tax1Val)+parseFloat(tax2Val)+parseFloat(tax3Val);
...
{
	setUpPageParameters (zcServletPrefix+"/custom/adhocReports/rAn.htm?pa=ir"+id,entityDiv);  
}
...
function getEODAlert(id, rowName, rowId)
{
	...
	document.getElementById("commonPopupDiv").innerHTML = "";
		...
		$('#commonPopupDiv').dialog({ title:'Periodic Alert'});
		if(rowId)
		document.getElementById(rowId).setAttribute("class", "rowSelectedClass");	
...
function processOzoneTel(userId,userName)
{
				...
				socket.on('processCall', function(jsonStr) {
                 ...
                 /*****Data to be posted******/
				 var actvtId;
				 var actvtIndex = uui.indexOf("actvt:");
				 if(actvtIndex !=-1) actvtId = uui.replace(/actvt:/gi,"");
				 var subject="";
				 var callType="";
				 ...
				 else if(actvtIndex !=-1 && type.toLowerCase() == "progressive") {
				 ...
				 var postData;
                 if(isNaN(actvtId)){
				 postData='0-1-3='+subject+'&0-1-96='+newDate+'&0-1-6=1&0-1-395='+escape(ucid)+'&0-1-398='+escape(did)+'&0-1-385='+escape(did)+'&0-1-387='+escape(monitorUcid)+'&0-1-389='+escape(callerID)+'&0-1-400='+escape(skillName)+'&0-1-413='+agentID+'&0-1-397='+campaignId+'&0-1-405='+type+'&0-1-396='+uui+'&0-1-124=main_call';
				 }
				 else {
					...
					success: function (data)
					{
					   ...
					   $.window.prepare({
						...
						//var phone_num=primPhone.replace("91","0");
								var url2Hit="/atCRM/custom/JSON/view/contact360View.htm?pri_phone="+primPhone;
								$.ajax({
									type: "GET",
									url:url2Hit,
									dataType: "json",
...
/* This function will push the contacts of the selected Mktg pgm,
   Also will add the generated campaign name*/
function pushCampaign(mktg_id)
{
	var evt_unq_str='mktg_'+mktg_id+'_'+Math.random();
	var campnName = "PriorityCampaign";
	var apiKey = "KK0cffeaeccf91b3c9daa05c5e90a9bf95";
	var d = new Date();
	var dataSrc=d.getDate()+"/"+(d.getMonth()+1)+"/"+d.getFullYear()+"-"+Math.random();
	var postString='mktg_id='+mktg_id+'&campnName='+campnName+'&apiKey='+apiKey+'&dataSrc='+dataSrc;
	var urlToHit = zcServletPrefix+'/custom/marketing/pushCampain2Ozonetel.json';
	$.ajax({
		url:urlToHit,
		type: "GET",
		data: postString,
		...
		success: function (data)
		{
			 var pushCnt = data["contMktgCount"];
			 var windWidth = $(window).width();
             windWidth = windWidth/2;
			 var infoDiv = document.getElementById("infoDiv");
			 infoDiv.innerHTML="";
             var infoDivHTML;
			 infoDivHTML = "<div style='display:block;font-size:11px;'>"+pushCnt+" Contacts have been pushed to Ozonetel.</div>";
			 infoDiv.innerHTML = infoDivHTML;
			 $("#infoDiv").show();
			 var divWidth = infoDiv.offsetWidth;
			 var div_half_Width = divWidth/2;
			 var postLeft = windWidth - div_half_Width;
			 infoDiv.style.left = postLeft+"px";
			 setTimeout("document.getElementById('infoDiv').style.display='none'",5000);
		},
		error:function()
		{
			alert("Error");
...
function showChangeCampaign()
{
}

function PrintOrEmail(url)
{
alert(url);
	var divContnt='';
	$.ajax({
		...
		success : function(tmplts){
			...
			//$("#commonPopupDiv").html("<input type='button' value='Print' ><input type='button' value='Email' );'><div width='850' >"+divContnt+"</div>");
			$("#commonPopupDiv").html("<input type='button' value='Print'  style='margin-left:10px;' onclick=\"javascript:printTemplate('"+url+"');\"><input type='button' value='Email' style='margin-left:10px;'  onclick=\"javascript:emailTemplate('"+url+"');\"><div width='850' >"+divContnt+"</div>");
...
{
	myWindow=window.open(url,'','width=900,height=800');
	resetEmailDiv();
}

function emailTemplate(url)
{
	if($("#sendMailDiv").length)
	{
		composeMail();
	}else{
		$('body').append("<div id='sendMailDiv' name='sendMailDiv' title='sendMailDiv' style='height:120px;display:none'></div>");
		composeMail();
	}
	resetEmailDiv();
}

function resetEmailDiv(maildiv)
{
	//$("#sendMailDiv").html='';
	//$('#sendMailDiv').dialog('close');
	$("#commonPopupDiv").html='';
	$('#commonPopupDiv').dialog('close');
	$(".ui-dialog-titlebar-close").show();
}
...
{
	var sendMailObj=document.getElementById('sendMailDiv');
	...
	sendMailObj.innerHTML="<iframe id='hintQuestion' frameborder='0' src='"+zcServletPrefix+"/custom/mails/sendEmailFromClient.html?is_compose=0&isCommonPopupDiv=1&PkValue="+PkValue+"&entityName="+tableName+"' width='100%' style='border:0px solid red;margin:0 auto;' height='550px'></iframe>";
}
function ClsCmnPpDv(x){
	if(x == 3)
		setTimeout(function(){
			$('#sendMailDiv').html("<h3>eMail has been sent.</h3>");
			$('#sendMailDiv').dialog('close');
		},3000);
	else	$('#sendMailDiv').dialog('close');
} 

function addTypeBasedPurchaseOrder()
{
	var statusURL = zcServletPrefix+"/custom/JSON/system/getPurchOrdStatuss.json";
	$.ajax({
		type: 'GET',
		dataType : 'JSON',
		url : statusURL,
		success : function(data){
		 //alert(JSON.stringify(data));
			var statusTypes = data['purchOrdStatus'];  
			var statusOptions = '';
			for (var i in statusTypes)
			{
				statusOptions += "<div class='popupOption' title='"+statusTypes[i].statusDesc+"'  onclick=\"javascript:addTypeBasedPurchaseOrder2("+statusTypes[i].statusId+",'"+statusTypes[i].statusDesc+"');\">"+statusTypes[i].statusDesc+"</div>";
			}
			$('#commonPopupDiv').dialog({
	...
	//var optionsHtml = "<div class='popupOption' title='Suppliers Quotation'  onclick=\"javascript:addTypeBasedPurchaseOrder2(1);\">Suppliers quotation</div><div class='popupOption' title='Purchase Order'  onclick=\"javascript:addTypeBasedPurchaseOrder2(2);\">Purchase order</div>";
	
} 

function addTypeBasedPurchaseOrder2(ord_status,statusName,status_desc)
{

	switch(ord_status)
	{
		case 3: case 4: 		
			var urlToPage=zcServletPrefix+"/custom/JSON/add/purchOrders.htm?cont_driven=0&ord_status="+ord_status+"&status_name="+statusName;
				renderUsingAddEditJS(urlToPage);
				break;
		case 1 :case 2 :
			var Title = 'Select '+status_desc+' type';
			var url2call = zcServletPrefix+"/custom/JSON/system/getPurchOrdTypes.json";
			$.ajax({
				type: "GET",
				url: url2call,
				dataType: "json",
				success: function (data) {
					var POtype = data['purchOrdType'];
					var htmlToAdd = '';
					for(var i in POtype )
					{	
						var urlToPage=zcServletPrefix+"/custom/JSON/add/purchOrders.htm?cont_driven=0&ord_type="+POtype[i].typeId+"&ord_type_name="+POtype[i].typeName+"&ord_status="+ord_status+"&status_name="+statusName;
						htmlToAdd += "<div class='popupOption' title='"+POtype[i].typeDesc+"'  onclick=\"javascript:renderUsingAddEditJS('"+urlToPage+"');\">"+POtype[i].typeName+"</div>";
					}
					$('#commonPopupDiv').dialog({
						title: Title,
						modal:true,
...
function renderUsingAddEditJS(pageUrl)
{	
	var $popupDiv = $('#commonPopupDiv')
	...
	$.ajax({
		...
		dataType: "json",
		success: function (data) {
			handleAddEditJsonData(data);
		}
	});	
}
// called from front end rule
function IGL_purchOrderCalcs(ele) 
{
	var elem = ele;
   if(G_data.UDMName == 'custom/JSON/add/purchOrders')
	{    
	    $("#grndTot_tbl").css('display','none');
		$('#footerTable').css({'display':'none'});
	
		$('#footerTable img').each(function(){
			src = $(this).attr('src');
			if(src.indexOf('delete-icon') > -1)$(this).click();
		});
		$('.totalDiv').html($('.totalDiv').html().replace('(Rs)','Cost'));
	
		var  regExz = /^0\-1501\:\d+\-401\-403$/; // ----  /0\-1501\:\d+\-401\-47[01]?[234]?[a-z]*/ 
		$('#listItemsTableBody input').on('blur',function(){//alert('id');
           if(regExz.test($(this).attr('id')))
		   {
			  calcTotal4PurchOrder($(this),5);
		   }
		});
		var regExz = /^0\-1501\:\d+\-401\-(47[0-4]|403)$/; // ----  /0\-1501\:\d+\-401\-47[01]?[234]?[a-z]*/ 
		$('#listItemsTableBody input').on('keyup',function(){//alert('id');
           if(regExz.test($(this).attr('id')))calcTotal4PurchOrder($(this),5);
		});
	   
	   $('#MoreLinkId').unbind('click'); 
		$("#MoreLinkId").on('click',function(){
			$('#listItemsTableBody input').unbind('blur');
			$('#listItemsTableBody input').unbind('keyup');
			IGL_purchOrderCalcs();
		});
	
		$("#listItemsTableBody > tr").each(function(index){
			if(!document.getElementById("0-1501:"+index+"-401-410"))
				$(this).append("<td align='left' class='dummyItm' style='display:none;'><input type='text' class='inputFieldClass' id='0-1501:"+index+"-401-410' name='0-1501:"+index+"-401-410' value='0'></td>");
				if(document.getElementById("0-1501:"+index+"-401-475"))
				document.getElementById("0-1501:"+index+"-401-475").disabled = true;
		});
...
/*function totalPurchOrderCalc(elem)
{
var orws = $(elem).closest('tbody').find('tr').length;//alert(orws);
var splitID = $(elem).attr('id').split(':');
var idPart = splitID[1].split('-');
//alert(idPart[2]);
for (i=0;i<orws;i++ )
{
    id= splitID[0]+":"+i+"-"+idPart[1]+"-"+idPart[2];
    var ele = document.getElementById(id);
    $(ele).attr('onblur','calcTotal4PurchOrder(this)');
    
}

}*/
/********* Calculate total for Add Purchase order : START : by venkatesh dp*******/
/**This code is used only for IGL , when custom fields values are added to line level total**/
function calcTotal4PurchOrder(qtyObj,RowCount)
{
	var splitID = $(qtyObj).attr('id').split(':');
	var idPart = splitID[1].split('-');

	var FAS = document.getElementById('0-1501:'+idPart[0]+'-401-470'); 
	var FRT = document.getElementById('0-1501:'+idPart[0]+'-401-471');
	var INT = document.getElementById('0-1501:'+idPart[0]+'-401-472');
	var CFR = document.getElementById('0-1501:'+idPart[0]+'-401-473');
	var FOB = document.getElementById('0-1501:'+idPart[0]+'-401-474');
	var unitPrice = document.getElementById('0-1501:'+idPart[0]+'-401-475');
	var total = document.getElementById('0-1501:'+idPart[0]+'-401-452');
	var product = document.getElementById('0-1501:'+idPart[0]+'-401-2201');
	var Qty = Number(document.getElementById('0-1501:'+idPart[0]+'-401-403').value);
   
   if((Qty > -1) && (product.value != "2 chars or **") && (product.value != ""))
	{  
		//if(!document.getElementById('0-1501:'+idPart[0]+'-401-403').value)
        //document.getElementById('0-1501:'+idPart[0]+'-401-403').value = 0;
		var CalcTotal = 0;
		if(FAS)var fasVal= FAS.value?FAS.value:0;
		if(FRT)var frtVal= FRT.value?FRT.value:0;
		if(INT)var intVal= INT.value?INT.value:0;
		if(CFR)var cfrVal= CFR.value?CFR.value:0;
		if(FOB)var fobVal= FOB.value?FOB.value:0;
		
		if(fasVal) CalcTotal+=Number(fasVal);
		if(frtVal) CalcTotal+=Number(frtVal);
		if(intVal) CalcTotal+=Number(intVal);
		if(cfrVal) CalcTotal+=Number(cfrVal);
		if(fobVal) CalcTotal+=Number(fobVal);
		
		if(unitPrice)unitPrice.value = CalcTotal;
        if(total)total.value = CalcTotal*Qty;
		prodTotalAmnt4Order['0-1501:'+idPart[0]+'-401-452']=CalcTotal*Qty;
		calculateProdTotalPriceForOrder();
	}
	
}

function IGL_invoiceCalcs()
{
	 if(G_data.UDMName == 'custom/JSON/add/invoices')
	{    
	   /* $("#grndTot_tbl").css('display','none');
		$('#footerTable').css({'display':'none'});*/
	...
	/*	$('#footerTable img').each(function(){
		...
		var  regExz = /^0\-1001\:\d+\-1101\-1103$/; // ----  /0\-1501\:\d+\-401\-47[01]?[234]?[a-z]*/ 
		$('#listItemsTableBody input').on('blur',function(event){//alert('id');
           if(regExz.test($(this).attr('id')))
		   {
				calcTotal4invoice($(this),event);
		   }
		});
		var regExz = /^0\-1001\:\d+\-1101\-(115[23]|1103)$/; // ----  /0\-1501\:\d+\-401\-47[01]?[234]?[a-z]*/ 
		$('#listItemsTableBody input').on('keyup',function(event){//alert('id');
           if(regExz.test($(this).attr('id')))calcTotal4invoice($(this),event);
		});
	   ...
	   $('#MoreLinkId').unbind('click'); 
		...
		$("#listItemsTableBody > tr").each(function(index){
			if(!document.getElementById("0-1001:"+index+"-1101-1127"))
				$(this).append("<td align='left' class='dummyItm' style='display:none;'><input type='text' class='inputFieldClass' id='0-1001:"+index+"-1101-1127' name='0-1001:"+index+"-1101-1127' value='0'></td>");
				
				if(document.getElementById("0-1001:"+index+"-1101-1154"))
				document.getElementById("0-1001:"+index+"-1101-1154").disabled = true;
                if(document.getElementById("0-1001:"+index+"-1101-1125"))
				document.getElementById("0-1001:"+index+"-1101-1125").disabled = true;
		});
...
var invProTotPrice = new Array();
function calcTotal4invoice(ele,event)
{
	//alert($(ele).value);
//	alert(event.which);
   // 0-1001:0-1101-2201
    var RowIndex = $(ele).attr('id').split(':')[1].split('-')[0];
    //alert(RowIndex);
    if(document.getElementById('0-1001:'+RowIndex+'-1101-2201').value != '2 chars or **' && document.getElementById('0-1001:'+RowIndex+'-1101-2201').value != '' && document.getElementById('0-1001:'+RowIndex+'-1101-2201').disabled == true)
    {
		var elQty = document.getElementById("0-1001:"+RowIndex+"-1101-1103");
		var elFob = document.getElementById("0-1001:"+RowIndex+"-1101-1152");
		var elFrt = document.getElementById("0-1001:"+RowIndex+"-1101-1153");
		var elTPPU = document.getElementById("0-1001:"+RowIndex+"-1101-1154");
		var elExtAmt = document.getElementById("0-1001:"+RowIndex+"-1101-1125");
		//alert(elQty.value?elQTY:0);
		var elQtyVal = Number(elQty.value?elQty.value:0);
		var elFobVal = Number(elFob.value?elFob.value:0);
		var elFrtVal = Number(elFrt.value?elFrt.value:0);
		//var elTPPUVal = parseFloat(Number(elTPPU.value?elTPPU.value:0)).toFixed(2);
		
		elTPPU.value = parseFloat(elFobVal + elFrtVal).toFixed(2);
		var tot = elExtAmt.value = parseFloat(elQtyVal * (elFobVal + elFrtVal)).toFixed(2);
		invProTotPrice['0-1001:'+RowIndex+'-1101-2201'] = tot;
		//document.getElementById('totalPrice').value=
		var grndTot = 0;
		for (var i in invProTotPrice)
		{
			grndTot = Number(grndTot)+Number(invProTotPrice[i]);
		}
		document.getElementById('totalPrice').value= parseFloat(grndTot).toFixed(2);
    }
...
/********* Calculate total for Add Purchase order : END*******/
...
function showPrice(srcZcRank)
{
	if((document.getElementById("0-1001:"+srcZcRank+"-1101-1104"))&&(document.getElementById("0-1001:"+srcZcRank+"-1101-6301"))&&(document.getElementById("0-1001:"+srcZcRank+"-1101-1125"))){
			var extndTotVal = document.getElementById("0-1001:"+srcZcRank+"-1101-1125").value;
			var taxAmt = document.getElementById("0-1001:"+srcZcRank+"-1101-6301").value;
			var qty = document.getElementById("0-1001:"+srcZcRank+"-1101-1103").value;
			var priceAmt = extndTotVal-taxAmt;
			priceAmt=priceAmt/qty;
		   document.getElementById("0-1001:"+srcZcRank+"-1101-1104").value = priceAmt.toFixed(2);
	}
}

/* $(function(){
//$("#listItemsTableBody :input").css({"height":"100%","margin":"0px","border-radius":"0px"});
//alert("End of pageLevelFunction.js");
}); */

/*function recalculate4Edit()
{
	var discAmt=0;
	if(document.getElementById('0-1-850')&&document.getElementById('0-1-850').value!="") discAmt=document.getElementById('0-1-850').value;
	for(var i=0;i<25;i++){
		var elem= document.getElementById("0-1001:"+i+"-1101-1103");
		calculateTotForInv("addEditForm",elem);
	}
    document.getElementById('0-1-850').value = discAmt;
    calculateTotForInv("addEditForm",document.getElementById('0-1-850'));
}*/