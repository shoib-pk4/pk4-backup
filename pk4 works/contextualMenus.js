...
{
	var url=zcServletPrefix+"/custom/Quote/getAjaxRespons4ViewQuote.json?HeaderId="+HeaderId+"&fileName="+filename
	$.ajax({
				type: "GET",
				url: url,
				dataType: "json",
	...
	if (divContent.indexOf("No matches")>-1 && searcStr.length==10)
	{
		...
		{  
			...
			if (resp)
			{
				...
				var post_val = { "0-1101": age, "0-1201": beneficiary, "0-1301": date_of_birth, "0-1401": employee_code, "0-1501": end_date , "0-1601": gender , "0-1701": maid, "0-1801": policy_description ,"0-1901": policy_holder_name ,"0-2001": policy_number ,"0-2101": primary_beneficiary ,"0-2201": relation ,"0-2301": response_timestamp ,"0-2401": start_date ,"0-2501": sum_insured ,"0-2601": policy_details ,"0-2701": policy_type ,"0-2801":insurer};
				var JSONURL=zcServletPrefix+"/custom/JSON/system/createContactFromAPI/editAction";
				$.ajax({
				type: "POST",
				url: JSONURL,
				data: post_val,
				...
				success: function (data)
				{
					...
					$.ajax({
						...
						success: function (doc)
						{
						   ...
						   if (document.getElementById("0-1-26")) {document.getElementById("0-1-26").value = contId; }
			...
			{
				alert('API retunred no data for the MAID number you have keyed in...');
			}
		},
		error: function() 
		{
			alert('Error retrieving MAID info from API...');
		}
		});
	}
}
...
function recieveTransitForInventory(pkVal)
{
	...
	$.ajax({
			...
			success: function(doc)
			{
				...
				$('.ui-dialog-title').html("Receive Inventory Goods");
				if(flag == 1)
				{
					var convertDivContent = "<table style='margin-top:5px; border:0px solid red;'cellpadding='3'>";
					convertDivContent+="<tr><td colspan='2'><div align='center' style='margin-top:30px;'><center>Would you like to Receive all goods  From <b>"+srcBrchName+"</b> To <b>"+destBrchName+".<br/><br/>Click <b>Yes</b> to Receive and <b>No</b> to cancel.</center></div><div align='center' style='margin-bottom:30px;'><tr><td align='center'><input style='width:70px;margin-bottom:10px;' class='greenButton' type='button' value='Yes' onclick='updateTransitForInventory("+pkVal+")'/>&nbsp;<input style='width:70px;margin-bottom:10px;' class='greenButton' type='button' value='No' onclick='javascript:closePopup("+pkVal+")'/></td></tr></div></td><tr>"
					convertDivContent +="</table></center>";   //</b> of batch number <b>'"+batchno+"'</b>
					document.getElementById("commonPopupDiv").innerHTML = convertDivContent;
				}
				else
				{
...
function saveAssignedGoods(wnd)
{
	...
	var isValid = validateOnSave();
	/*Check the qty assigned for the batch with available qty*/
	if(isValid){
		var postData="";
		for(var i=0;i<invDtlData.length;i++)
		 {
...
/*This function is to check report error if the qty assigned more than the available batch qty*/
function validateOnSave()
{
	var invDtlData=invoiceData["invoice_detail"];
	var postData="";
	/*Loop through invoice detail data*/
	for(var i=0;i<invDtlData.length;i++)
	 {
        var invDtl_id = invDtlData[i]["id"];
        var prodBrchInvtDtl = invDtlData[i]["prod_branch_invt_detail"];
		var trElems = document.getElementsByClassName("tr_"+invDtl_id);
		var trsLen = trElems.length;
		/*Loop through the trs to get the assigned batch and qty*/
		for(var j=1;j<parseInt(trsLen)+1;j++)
		  {
			 for(var k=1;k<4;k++)
				 {
					batchTxtId= "bch_txt"+j+"_"+k+"_"+invDtl_id;
					qtyTxtId  ="qty_"+j+"_"+k+"_"+invDtl_id;
					/*if the batch and qty not null*/
					if(document.getElementById(batchTxtId).value !="" && document.getElementById(batchTxtId).value !="2 chars or **"&& document.getElementById(qtyTxtId).value !="" && document.getElementById(batchTxtId).value)
					 {
						var batchNum = document.getElementById(batchTxtId).value;
						var qty = document.getElementById(qtyTxtId).value;
						/*Loop throught the branch invt detail to check the qty availability. If qty is > than the availability than show error*/
						for(var a=0;a<prodBrchInvtDtl.length;a++)
						  {
							var batchNumber = prodBrchInvtDtl[a]["batchNumber"];
							var invtryQty = prodBrchInvtDtl[a]["invtryQty"];
							if((batchNum === batchNumber) && (parseInt(qty) > parseInt(invtryQty))){
...
function validateBatchNQty(qtyElem)
{
	 ...
	 var product = document.getElementById('prod_id_'+invDtlId).value;
	 var totQty=0;
	...
	for(var i=0;i<invDtlData.length;i++)
	{
		...
		{
		   var invtProdId = prodBrnchInvtDtl[m]["productId"];
		   ...
		   if(batchNum == batchVal && product==invtProdId && parseInt(invtryQty)<parseInt(qtyVal)) {
...
function validateBatch(batchElem)
{
	...
	for(var i=0;i<invDtlData.length;i++)
	{
	  ...
	  var listPrice = invDtlData[i]["list_price_per_unit"];
	  var productIdName = invDtlData[i]["product"];
	  var prodName = productIdName.split("--")[1];
	  var batchFnd="0";
	  if(invDtlId == prodBrchInvDtlId){
		for(var m=0;m<prodBrnchInvtDtl.length;m++)
		{
           var batchNum = prodBrnchInvtDtl[m]["batchNumber"];
		   var mrp = prodBrnchInvtDtl[m]["mrp"];
		   if(batchNum == batchVal){
			   if(mrp !="" && listPrice < mrp){ document.getElementById("error_div").innerHTML ="Product with batch number "+batchNum+" has higher price(Rs."+mrp+") than the invoice product price (Rs."+listPrice+")";
		       //setTimeout("document.getElementById('error_div').innerHTML=''",15000);
			   }
			   else if(mrp !="" && listPrice > mrp){ document.getElementById("error_div").innerHTML ="Product with batch number "+batchNum+" has lower price(Rs."+mrp+") than the invoice product price (Rs."+listPrice+")";
		       //setTimeout("document.getElementById('error_div').innerHTML=''",15000);
			   }
...
function fillMAIDRelFlds(){
	...
	if (contId!='')
	{
		...
		$.ajax({
			...
			success: function (doc)
			{
               var policynumFrmMAID = doc["policynum"];
			   var policytypeFrmMAID = doc["policytype"];
			   var insurerFrmMAID = doc["insurer"];
			   var corpIdFrmMAID = doc["corpId"];
			   var corpstrFrmMAID = doc["corpstr"];
			   var mobileFrmMAID = doc["mobile"];
			   var emailFrmMAID = doc["email"];
			   var policyDetFrmMAID = doc["policyDet"];
			   if (MAID != '0000000001' && MAID != '0000000000' && (policynumFrmMAID == '' || !policynumFrmMAID))
			   { 
				   var JSONURL="http://mediassistindia.com/CRMServices/Default.aspx?MAID="+MAID;
					$.ajax({
					type: "GET",
					url: JSONURL,
					dataType: "jsonp",
				...
				{//alert('am i in');
					if (document.getElementById("0-1-113")) { document.getElementById("0-1-113").value = policytypeFrmMAID; document.getElementById("0-1-113").disabled = "true"; }
					if (document.getElementById("0-1-114")) { document.getElementById("0-1-114").value = insurerFrmMAID; document.getElementById("0-1-114").disabled = "true"; }
				    if (document.getElementById("0-1-32txt")) { document.getElementById("0-1-32txt").value = corpstrFrmMAID; document.getElementById("0-1-32txt").disabled = "true"; }
					if (document.getElementById("0-1-32")) { document.getElementById("0-1-32").value = corpIdFrmMAID; document.getElementById("0-1-32").disabled = "true"; }
					if (document.getElementById("0-1-121")) { document.getElementById("0-1-121").value = policynumFrmMAID; document.getElementById("0-1-121").disabled = "true" ; }
					if (document.getElementById("0-1-116")) document.getElementById("0-1-116").value = mobileFrmMAID;
				    if (document.getElementById("0-1-115")) document.getElementById("0-1-115").value = emailFrmMAID;
					if (document.getElementById("0-1-120")) { document.getElementById("0-1-120").value = policyDetFrmMAID; document.getElementById("0-1-120").disabled="true"; }
...
function sendEmailWithAttachment(pk)
{
	/*temp values*/
	var fileName='vinutha@pk31072013011324.pdf';
	var fromName='venkatesh@vdps.twomini.com';
	var contId ='';
	/*temp values ends*/
	//alert(pk);
	document.getElementById("commonPopupDiv").innerHTML = "";
	...
	var convertDivContent = "<table style='margin-top:5px; border:0px solid red;' cellpadding='3'>";
	convertDivContent+= "<iframe src='"+zcServletPrefix+"/custom/mails/finalSendMail.html?is_compose=1&exterAttchFile="+fileName+"&fromName="+fromName+"&contId="+contId+"&fromOrder=1' frameborder='0' width='825px' height='565px' style='border:0px solid red;'></iframe>";
	convertDivContent += "</table></center>";
	document.getElementById("commonPopupDiv").innerHTML = convertDivContent;
}
...
function closeThis()
{
	//alert('close manually');
		...
		$('#commonPopupDiv').html("<center>eMail sent successfully</center>");
		setTimeout(function(){$('#commonPopupDiv').dialog('close')}, 2000);
		//alert('close manually');
}
...
function popCancelReasons(enttName){
	pk1=pk;
    $.ajax({
    type: "GET",
    url:zcServletPrefix+"/custom/JSON/system/listObjectCancelReason.json",
    data: "enttName="+enttName,
    timeout: "30000",
    dataType: "json",
    success:function()
	{
		var CancelReasons = doc["CancelReasons"];
		var allCancelReason = CancelReasons.length;	
		if(allCancelReason>0)
		{
				var convertDivContent = "<table style='margin-top:5px' cellpadding='3'>";
				convertDivContent += "<tr><td width='100px' class='flexigrid'>Assign to</td><td>: ";
				var tempSelectTerr = "<input type='hidden' name='reasonId' id='reasonId'><select name='allReasons' id='allReasons' class='default' onchange='assignReasonId(this)'><option value=''>&nbsp;&nbsp;&nbsp;--&nbsp;&nbsp;None&nbsp;&nbsp;&nbsp;--&nbsp;&nbsp;</option>"
				for(k=0,l=0;k<allCancelReason;k++,l++)
				{									
					reason_id = CancelReasons[l]["Reason_id"];				
					reason = CancelReasons[l]["Reason"];
					tempSelectTerr += "<option value='"+reason_id+"'>"+reason+"</option>";
				}
				tempSelectTerr += "</select>";
				convertDivContent+= tempSelectTerr;
				convertDivContent+= "</td></tr>";
		...
		{		
			var convertDivContent = "<table align='center' style='margin-top:5px; border:0px solid red;' cellpadding='3'>";
			convertDivContent += "<tr><td colspan='2' class='flexigrid'><br/><br/><b>Please setup Cancel reasons...</b><br/><br/></td></tr>"; 
			convertDivContent += "</table></center>";
...
function assignReasonId(obj){
	var reasonId = obj.value;
	alert(reasonId);
}
