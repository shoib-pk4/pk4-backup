...
{	
	...
	var mainTD=document.getElementById(mnuItmId+"-MainTd");
	var rowNum;
	if(recId) rowNum=data.quoteDtlRows;
	else rowNum=5;
	if(ListPanelHeaders)
	{
...
function submitAddEditForm(formSubmitPostFun,isAddNew)
{
	...
	success: function (data)
	{
		...
		if(data){
			doc = JSON.parse(data);
			var addedId = doc.addedId;
			var addedTxt = doc.addedTxt;
			var respAftrAdd = doc.respAftrAdd;
			if(addData && isAddNew == "addNew")
              ...
              {  
				if(formSubmitPostFun){
					...
					if(respAftrAdd =="0") responsePage(mnuItmId,fromPage,true,action,currPageUDM,formSubmitPostFun);
					else responsePage(mnuItmId,fromPage,true,action,currPageUDM,formSubmitPostFun,"noResponse");
			    }
			    else responsePage(mnuItmId,fromPage,true,action,currPageUDM,formSubmitPostFun);
			  }
		    if(document.getElementById("attchedId").value !="")
		      {
				var attachedIds = document.getElementById("attchedId").value;
				var lnkUrl= zcServletPrefix+"/custom/JSON/admin/linkEntity2LibItms.html?pk="+addedId+"&entity="+entity+"&libIds="+attachedIds;
                $.ajax({
				  type: "GET",
				  url: lnkUrl,
				  success: function (doc)
	    ...
	    if(windowData) windowData.restore();
	},
	error: function ()
		{ 
			if(addData && isAddNew == "addNew")
...
function unblockMappedObjectFields(thisElemId,thisTblName,formName)
{
	...
	if(document.getElementById(thisElemId+'_unlock').style.visibility=="visible")
	{
		...
		{
			 if(document.getElementById("0-1-251txt")){
				 document.getElementById("0-1-251txt").value = "2 chars or **";
