...
function plotDataTable(data) {
	...
	oTable = $('#reportData').dataTable({
			...
			"bLengthChange": true, 
			// "sScrollX": "100%",
			"bStateSave": true,//saves the states, if cookie is bing may cause error
			"oLanguage": {
	...
	//$('#reportData_length, #reportData_filter').appendTo('#tabsContainer');
}
		...
		for(var i=0, fc="dpass"; i<colHdrLen; i++, fc="") {
			...
			if($.inArray(dataTblHdr[i]['sClass'], colTotReq) != -1 ) {
				...
				for(var j=0; j<colDtLen; j++) {
					toNum  = parseInt(dataTblData[j][i]);
					if(toNum) 
						t += toNum;
				}
				t = Math.round(t);
				td = '<td style="text-align:right;" class="dtTotal'+fc+'">'+ t + '</td>'	
			} 
			else {
				td = '<td class="'+fc+'">'+ t + '</td>';
			}
