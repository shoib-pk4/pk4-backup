...
function plotDataTable(data) {
	...
	oTable = $('#reportData').dataTable({
			...
			"sDom": 'T<"clear">lfrtip',
			"aaSorting": [[0, 'asc']],
			"bLengthChange": true, //saves the states, if cookie is bing may cause error
			// "sScrollX": "100%",
			"bStateSave": true,
			"oLanguage": {
...
function addTrForShowingTotal() {
	if(totalColCntDt == true) {
		...
		var tr = $('<tr class="dtTotal"></tr>'), td, t='-', fc;
		for(var i=0, fc="dpass"; i<colHdrLen; i++, fc="") {
			...
			if($.inArray(dataTblHdr[i]['sClass'], colTotReq) != -1 ) {
				...
				for(var j=0; j<colDtLen; j++) {
					t += parseInt(dataTblData[j][i]);
				}
				t = Math.round(t);
				td = '<td style="text-align:right;" class="'+fc+'">'+ t + '</td>'	
			} 
			else {
				td = '<td class="'+fc+'">'+ t + '</td>';
			}
