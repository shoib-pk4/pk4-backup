...
function printObject (enttName,pkId) {
	var entt_id = '';
	//alert(enttName+" : "+pkId);
	if(enttName == 'undefined')enttName='order_header';// hard coded
...
{	
	...
	$.ajax({
		...
		$("#commonPopupDiv").html("<input type='button' value='Print' onclick=printTemplate('"+url+"')><input type='button' value='Email' onclick=emailTemplate('"+url+"')><div width='850' >"+divContnt+"</div>");
...
function composeMail()
{
	...
	sendMailObj.innerHTML="<iframe id='hintQuestion' frameborder='0' src='"+zcServletPrefix+"/custom/mails/sendEmailFromClient.html?is_compose=1&isCommonPopupDiv=1' width='100%' style='border:0px solid red;margin:0 auto;' height='550px'></iframe>";
}
...
function addTypeBasedPurchaseOrder2(ord_status)
{
var Title = "";
switch(ord_status)
{
	case 1: Title='Select suppliers quotation type';
			break;
	case 2: Title='Select purchase order type';
			break;
}
var url2call = zcServletPrefix+"/custom/JSON/system/getPurchOrdTypes.json";
 $.ajax({
	...
	success: function (data) {
		...
		for(var i in POtype )
		{	
			var urlToPage=zcServletPrefix+"/custom/JSON/add/purchOrders.htm?cont_driven=0&ord_type="+POtype[i].typeId+"&ord_type_name="+POtype[i].typeName+"&ord_status="+ord_status;
	...
	/*for(var i in POtype){ 
		...
		$optionsDiv.append("<div class='popupOption'>"+POtype[i]+"</div>");
	}*/
}
...
function calcTotal4PurchOrder(qtyObj,RowCount)
{
	...
	var ListPrice = document.getElementById('0-1501:'+idPart[0]+'-401-410');
	var total = document.getElementById('0-1501:'+idPart[0]+'-401-452');
	//if(CFR)alert("cfr");//ListPrice.style.display = 'none';
	var ListPriceVal = ListPrice?ListPrice.value:0;
	var Qty = Number(document.getElementById('0-1501:'+idPart[0]+'-401-403').value);
     if(Qty && (Qty > -1))
	 {
		...
		CalcTotal = CalcTotal + Number(ListPriceVal);
        total.value = CalcTotal*Qty;
