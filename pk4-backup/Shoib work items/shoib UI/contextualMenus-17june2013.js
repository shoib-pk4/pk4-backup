...
function getSmartSuggDiv(){
	...
	{
		document.getElementById("smartSuggestDiv").innerHTML = 'Fetching data from your server, please wait...';
		...
		{  
			...
			success: function (data)
			{
				var policynum= data["policynum"];
				var policytype = data["policytype"];
				var insurer = data["insurer"];
				var polHoldId = data["polHoldId"];
				var polHoldName = data["polHoldName"];
				var mobile = data["mobile"];
				var email = data["email"];
				var maid = data["maid"];
				var contId = data["contId"];
				var policyDet = data["policyDet"];

				if (document.getElementById("0-1-26txt")) document.getElementById("0-1-26txt").value = maid;
				...
				if (document.getElementById("0-1-32txt")) { document.getElementById("0-1-32txt").value = polHoldName;document.getElementById("0-1-32txt").disabled="true"; }
				if (document.getElementById("0-1-32")) { document.getElementById("0-1-32").value = polHoldId;document.getElementById("0-1-32").disabled="true"; }
					...
					$.window({
						...
						onShow: function(wnd) {
								   ...
								   for(var i=0;i<invDtlData.length;i++)
								   {
                                      ...
                                      var invDtlRow=CreateTR(invDtTbody);
                                      ...
                                      var invDtlRow2=CreateTR(invDtTbody);
			...
			for(var k=1;k<4;k++)
			 {
				...
				var qtyNum =0;
				if(document.getElementById(qtyId).value) qtyNum=document.getElementById(qtyId).value;
	...
	for(var i=0;i<invDtlData.length;i++)
	{
		...
		for(var m=0;m<prodBrnchInvtDtl.length;m++)
		{
		   ...
		   if(batchNum == batchVal && parseInt(invtryQty)<parseInt(qtyVal)) {
			   //alert("Only "+invtryQty+" of "+prodName+" available.");
