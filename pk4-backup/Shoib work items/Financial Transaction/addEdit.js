...
{	
	...
	{	
		...
		if(data.UDMName=="custom/JSON/add/invoices")
		{
			...
			//If edit get the discounr percent value and assign it to the disc pct elemt
			...
			//If edit get the discounr percent value and assign it to the disc pct elemt
			if(priKey !="" && document.getElementById("0-1-850_hdn")) disc_amt.value=document.getElementById("0-1-850_hdn").value;
		}
		...
		if(data.UDMName == 'custom/JSON/add/purchOrders')
		{
			$('#totalCollection').closest('td').css({'visibility':'hidden'});
		}
		//var listTh=CreateTH(listHdTr,'TblHead','','#','center');
...
function builtMultiAddListFields(data)
{
	...
	alert(trRowCount);alert(ListPanelRows.length);
...
function blockMappedObjectFields(thisElemId,hdnFld)
{
	...
	document.getElementById(thisElemId+'_unlock').style.visibility="visible";
	var detUrl=zcServletPrefix+'/custom/JSON/smartSuggest/smartFillData.htm?cols='+cols+'&tblName='+fillTblName+'&pkCol='+pkCol+'&pkVal='+pkVal;
	/***Pass brchId if tblName is Product, by Dony 2 Aug 2013****/
