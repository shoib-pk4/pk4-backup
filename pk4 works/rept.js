...
function initDataTable(params, doc) {
	...
	var reptFeture = (doc.ReportFeatures !== undefined)?doc.ReportFeatures:'';
	drawParamsConditions(params, reptFeture);
...
function plotDataTable(data) {
	...
	if(_reptPaginationStatus == 'increment') {
		...
		str = ' (Showing 1 - ' +  _reptCurrentResultCnt + ')';
		_reptPaginatioArr.push(str);
	}
...
*/
function drawParamsConditions(paramsData, reptFeture) {
...
$("#k_results").children().remove(); //empty if any
 ...
 var getPParm = getValueFromUrl('pa');
	if(getPParm == '' || getPParm === undefined){
		paUrl = '&pa='+paUrl;
	}
 else{
	var getPParmArr = getPParm.split('!~~!');
...
function reptShowColumnsForSingleSelect () {
	...
	var udm = '/atCRM/custom/metadata/eC.html?n='+reptEntityName;
	// console.log('filters...');
	...
	$.ajax({
		...
		success: function(data) {	
			...
			//data is empty then show nothing
			if(reptEntityColumns.length == 0) {				
				return;
			}
