/*  From github.com/nicolaskruchten/pivottable  */
(function() {
  ...
  */
  $.fn.pivotUI = function(input, opts) {
        ...
        (function(c) {
          ...
          if (numKeys > 20) {
            ...
            for (_l = 0, _len4 = _ref2.length; _l < _len4; _l++) {
              ...
              v = axisValues[c][k];
              filterItem = $("<label>");
              filterItem.append($("<input type='checkbox' class='pvtFilter'>").prop("checked", true).data("filter", [c, k]));
    ...
    var newAggTxt= ['Average', 'Count', 'Count Unique', 'Int Sum', 'List Unique', 'Sum'];
    var nac = 0; //new aggregator count
    for (x in _ref2) {
      if (!__hasProp.call(_ref2, x)) continue;      
      aggregator.append($("<option>").val(x).text(newAggTxt[nac]));
      nac++;
    }
    ...
    if (opts.effectsName != null) {
      $("#effects_" + (opts.effectsName.replace(/\s/g, ""))).prop('checked', true);
    }
    refresh = function() {
